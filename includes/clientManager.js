let CONST = require('./const'),
    ...
    dbGateway = require('./databaseGateway');
...
class Clients {
    ...
    async clientConnect(connection, clientID, clientData) {
        ...
        let client = await dbGateway.getClient(clientID);
        if (!client) {
            await dbGateway.addClient({
                clientID,
                firstSeen: new Date(),
                lastSeen: new Date(),
                isOnline: true,
                dynamicData: JSON.stringify(clientData)
            });
        } else {
            await dbGateway.updateClient(clientID, {
                lastSeen: new Date(),
                isOnline: true,
                dynamicData: JSON.stringify(clientData)
            });
